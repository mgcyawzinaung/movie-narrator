...
### v1.2 — 引擎可靠性与可观测性（下一版本）

> 主题：在任何 DAG 或分布式工作流工作之前，先闭环渲染可靠性、崩溃恢复与逐步骤可观测性。所有改动均在既有质量门禁（90% 覆盖率、mypy、ruff、SAST）下交付。预计不改变契约面 — `CONTRACT_VERSION` 保持 `(1, 0, 0)`。

#### 渲染与队列稳定性

- 可终止子进程 — 为 MoviePy 主编码与 sidechain BGM 混音补充 deadline 与进程组 terminate/kill（mux 已有超时）；记录 PID 与超时原因；清理残缺产物。
- 原子产物发布 — 渲染与片段产物先写任务临时路径，QA 通过后原子替换到最终路径；失败时删除残缺文件。
- 编码配置统一 — `render_video_codec` / `render_encoder` 在最终渲染与片段导出中一致生效（修复失效的配置路径）。
- 竖版 QA 修复 — `1080x1920` 输出不再被交付 QA 的宽高假设误判。
- 崩溃恢复闭环 — 启动扫描回收孤儿 `RUNNING` 任务（标记可恢复或失败），无需人工清理；端到端验证检查点恢复链路。

#### 服务加固

- 非 loopback 默认强制鉴权 — 外部绑定部署下拒绝匿名任务提交与产物下载。
- 任务准入限制 — 提交体大小、并发、单任务时长与估算产物大小上限。

#### 可观测与恢复可信度

- 结构化步骤日志 — 每个步骤输出任务 ID、步骤、尝试次数、耗时、Provider、缓存命中、PID、产物大小与错误分类。
- 执行清单 — 每个交付物记录输入、配置、Provider、逐步骤耗时、缓存命中、校验和与 QA 结果。
- 检查点指纹 — 输入/配置/Provider 指纹 + 产物清单 + schema 版本；恢复时失效过期检查点。
- OpenTelemetry 追踪（或等价的结构化 trace）— 任务为 trace，步骤/Provider/子进程为 span，附带延迟、重试、缓存与成本归因。
- 生成 dry-run — 仅研究 / 脚本 / 分镜规划，不调用 TTS 或 FFmpeg（与现有清理 dry-run 分离）。
- Provider 调用治理 — LLM / VLM / TTS / TMDB 统一采用重试/退避框架，设按 Provider 的预算，并以幂等键防止重复计费。

#### 效率与成本

- 硬件编码产品化 — 统一 FFmpeg 二进制与设备探测、能力缓存、回退原因上报与基准测试。
- 提示词/脚本缓存 — 以规范化主题 / 风格 / 语言 / 提示模板版本 / 模型为键，记录命中来源。
- TTS 缓存核算 — 跨任务命中率、引用计数与可审计来源。
- 资源感知准入 — 渲染前检查临时磁盘 / CPU / GPU / 时长 / 分辨率上限；GPU 与 CPU 队列分离。
...
以下特性保持在 v1.2/v1.3 范围之外，仅在社区反馈和企业需求明确后推进：
